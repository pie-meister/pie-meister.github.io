  ...
  svg(
    ...
    setTimeout((_) => {
      ...
      this.$ = [...root.querySelectorAll`slice`].map((sliceDefinition, idx) => {
        ...
        //! store dashoffset for the START of this slice
        ...
        group.create = (
          ...
          __stroke_dashoffset = slice_dashoffset,
              ...
              : path.getPointAtLength(
                  path.getTotalLength() -
                    (path.getTotalLength() / pathLength) *
                      (offset + (slice_dashoffset + __sliceSize / 2))
                )),
        ...
        // ------------------------------------------------------------------ create SVG slice
        //! let defined above to save let statement
        group.path = group.create(0); // path stroke positioning 0 = middle of slice

        //! 43 Bytes for Events
        group.onmouseout = (_) => (
          (this.slice = group),
          this.dispatchEvent(new Event(`slice`), group.pull(0))
        );
        group.onmouseover = (_) => (
          (this.slice = group),
          this.dispatchEvent(new Event(`slice`), group.pull(1))
        );
        ...
        //! calculate dashoffset ONCE for each slice, I tried to parameterize to for the create Slice function
        ...
        [...sliceDefinition.attributes].map((x) =>
          group.path.setAttribute(x.name, x.value)
        );
        ...
        group.pull = (
          ...
          pt = group.path.point(0, Math.abs(~~sliceDefinition.getAttribute`pull` || pull))
        ) =>
          group.setAttribute(
            `transform`,
            (group.pulled = state) // SET pulled property 
              ? `translate(${
                  pt.x - group.path.point().x
                } ${pt.y - group.path.point().y})`
              : `` //`translate(0 0)`
          );
        // ------------------------------------------------------------------ pull current slice
        group.pull();
