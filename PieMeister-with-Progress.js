    ...
    connectedCallback() {
        setTimeout((() => {
            ...
            let progressSlice = (t, e, i) => {
                ...
                let label = i.length > 1 ? "<tspan part=label x=59% >" + t.innerHTML + " </tspan><tspan part=value> $size</tspan>" : "" == t.innerHTML ? `<tspan part=value x=50% y=62% ${s ? "font-size=6em fill=" + s : ""}>$size</tspan>` : "<tspan part=label x=50% y=330>" + t.innerHTML + "</tspan>" + "<tspan part=value x=50% y=510 font-size=5em >$size</tspan>";
            ...
            this.innerHTML = "<style>" + "path{stroke-linecap:round}" + (t.length > 1 ? "text{text-anchor:end;alignment-baseline:middle;font-size:2em}" + "[part='text'] tspan:nth-child(2){font-size:1.5em;fill:white;font-weight:bold}" : `text{text-anchor:middle;alignment-baseline:middle;font-size:2.5em;fill:${this.getAttribute("color") || ""}}` + `[part="label"]{font-size:2em}`) + "[part='background']{opacity:0.3}" + "</style>" + this.innerHTML + (this.getAttribute("value") ? (this.setAttribute("range" + 10 * ~~(this.getAttribute("value").replace("%", "") / 10), this.getAttribute("value")), 
