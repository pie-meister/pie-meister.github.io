    ...
    setTimeout((l) => {
      ...
      [...t.querySelectorAll`slice`].map((t, l) => {
        ...
        o.create = (
          ...
          A.setAttribute(`stroke-width`, l),
          A.setAttribute(`pathLength`, r),
          A.setAttribute(`stroke-dasharray`, n + ` ` + (r - n)),
          A.setAttribute(`stroke-dashoffset`, a + n),
          A.setAttribute(
            `d`,
            `m${500 + i / 2} ${500 + i / 2}m0 ${-p}a2 2 0 000 ${
              2 * p
            }a2 2 0 000-${2 * p}`
          ),
        ...
        h == a
          ? r ||
            [...this.querySelectorAll`slice`].map(
              (t) => (r += ~~t.getAttribute`size`)
            )
          : (r = ~~this.getAttribute`size` || 100);
        h || ((h = r - s), (a = h + `%`));
        u = o.create(0);
        o.point = u.point;
        o.setAttribute(`slice`, l);
        o.setAttribute(`size`, a);
        o.setAttribute(
          `label`,
          (n.innerHTML = this.querySelector`style`
            ? (t.innerHTML && t.innerHTML.replace(`size`, a)) || a
            : ``)
        );
        u.setAttribute(`stroke-dashoffset`, (s += h));
        [...t.attributes].map((t) => u.setAttribute(t.name, t.value));
        n.setAttribute(
          `y`,
          o.point(
            0,
        ...
        let p = o.point(0, Math.abs(~~t.getAttribute`pull` || i));
        t.hasAttribute`pull` &&
          o.setAttribute(
            `transform`,
            `translate(${p.x - o.point().x} ${p.y - o.point().y})`
          );
      });
    });
  }
